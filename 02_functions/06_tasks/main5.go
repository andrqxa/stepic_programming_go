...
t=6w,w=km,m=p∗vt= \frac{6}{w}, w = \sqrt{\frac{k}{m}}, m=p*v
t=w6​,w=mk​

​,m=p∗v


ВАЖНО! считайте что пакет main уже объявлен, а также функция main() с вызовом ВАШЕЙ будущей функцией  T() уже есть. Несмотря на то что тестирование будет через ввод-вывод вам НЕ требуется вводить и выводить что-либо. Для подсчета используйте УЖЕ ВВЕДЕННЫЕ ГЛОБАЛЬНЫЕ переменные k,p,v ТИПА float64 !!!
